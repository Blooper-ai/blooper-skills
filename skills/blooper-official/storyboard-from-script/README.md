...
`Characters` and `Locations` tag assets stay the single source of truth — the
...
- You already have (or plan to generate) your cast in the `Characters` tag and want
...
- No new files in the `Characters` or `Locations` tags — deliberate; see above.
- The skill reports the new `storyboard_folder_id` and scene / shot counts
  when it finishes.

## Recommended follow-up

1. Make sure every character in the cast has an image in the `Characters` tag
